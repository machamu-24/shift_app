...
## URL
[https://shift-app-gpd8.onrender.com/]

## テスト用アカウント
*   **管理者用メールアドレス**: `staff_1@example.com`
*   **パスワード**: `password1`
*   **スタッフ用メールアドレス**: `staff_5@example.com`
*   **パスワード**: `password5`
...
## アプリケーションを作成した背景
医療現場のシフト管理は、必要出勤者数や連勤上限、専従スタッフの設定など、複雑な条件を考慮する必要があります。
...
## 実装予定の機能
*   LINEやslackと連携し、通知機能を追加（シフト確定時や提出期限前）
*   スタッフごとのチーム、専従スタッフ管理と、それに基いた自動割当ロジックの強化
...
bin/rails s
```
これで `http://localhost:3000` にアクセスしてください。
